...
pub async fn initialize(context: Context) {
  ...
  match std::env::var("ADMIN_GUILDS") {
    Ok(val) => {
      ...
      for id in ids {
        if let Err(why) = id
          .set_commands(&context.http, vec![broadcast::register()])
          .await
        {
          println!(
            "Could not register admin commands for guild {}: {why}",
            id.to_string()
          );
        }
      }
    ...
    Err(std::env::VarError::NotUnicode(_)) => {
      println!(
        "ADMIN_GUILDS environment variable has invalid data. Skipping adding Admin Commands"
      );
    }
  }

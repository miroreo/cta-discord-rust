...
use serenity::all::{
  ComponentInteraction, ComponentInteractionDataKind, Context, CreateAttachment, CreateButton,
  CreateCommandOption, CreateEmbed, CreateInteractionResponse, CreateInteractionResponseMessage,
  CreateSelectMenu, CreateSelectMenuKind, CreateSelectMenuOption, InteractionResponseFlags,
};
...
pub async fn select(ctx: &Context, component: &ComponentInteraction) -> CreateInteractionResponse {
  if let ComponentInteractionDataKind::StringSelect { values } = &component.data.kind {
    CreateInteractionResponse::UpdateMessage(
      arrivals_command(ctx, values.first().unwrap_or(&String::new()).as_str()).await,
    )
  } else {
    CreateInteractionResponse::Message(
      CreateInteractionResponseMessage::new()
        .ephemeral(true)
        .content("An error occured while attempting to select the stop."),
    )
  }
}

pub async fn refresh(ctx: &Context, component: &ComponentInteraction) -> CreateInteractionResponse {
  let last_time = component
    .message
    .edited_timestamp
    .unwrap_or(component.message.timestamp)
    .clone()
...
async fn arrivals_command(ctx: &Context, search: &str) -> CreateInteractionResponseMessage {
  ...
  for stop_id in found_stop_ids {
    let stop = gtfs
      .gtfs_data
      .get_stop(stop_id.as_str())
      .expect("Search result is invalid.");
  ...
  if stations.len() > 25 {
    return CreateInteractionResponseMessage::new()
      .content(
        "Too many results found for that station name. Please narrow your search.".to_string(),
      )
      .flags(InteractionResponseFlags::EPHEMERAL);
  } else if stations.len() > 1 {
    let select_menu_options: Vec<CreateSelectMenuOption> = stations
      .into_iter()
      .map(|stop| {
        let name = stop
          .name
          .unwrap_or_else(|| format!("Station ID {}", stop.id));
  ...
  let station = stations
    .first()
    .expect("No first station. This should not be possible to reach.");
  let predictions = tt
    .arrivals(ArrivalsParameters {
      id: cta::traintracker::MapOrStopID::MapID {
        mapid: stations.first().unwrap().id.parse().unwrap(),
      },
      max: None,
      rt: None,
    })
  ...
  match predictions {
    Ok(prds) => {
      let arrivals: Vec<Arrival> = prds
        .into_iter()
        .map(|prd| Arrival {
          destination_name: prd.destination_name,
          route: prd.route.into(),
          countdown: util::countdown(util::minutes_until(
            prd
              .arrival_time
      ...
      let png_data = arrivaldisplay::render_doc(&arrivaldisplay::train(
        format!(
          "Upcoming Arrivals for {}",
          station
            .name
            .clone()
            .unwrap_or_else(|| format!("Station ID {}", station.id))
        ),
          ...
          CreateInteractionResponseMessage::new()
            ...
            .button(
              CreateButton::new(format!(
                "arrivals:refresh/stationName/{}",
                station.name.clone().unwrap()
              ))
...
pub fn register() -> CreateCommand {
  ...
  let station_name = CreateCommandOption::new(

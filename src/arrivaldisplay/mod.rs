...
pub fn train(stop_description: String, arrivals: &[Arrival]) -> Document {
  ...
  for (i, arr) in arrivals.iter().enumerate() {
    ...
    document = document.add(
      Text::new(arr.destination_name.clone())
        .set("x", 25)
    ...
    document = document.add(
      Text::new(format!(
        "{} #{:0>3} to",
        to_variant_name(&arr.route).unwrap(),
        arr.train_number.to_string()
      ))
      .set("x", 25)
...
pub fn render_doc(doc: &svg::Document) -> Result<Vec<u8>, ArrivalDisplayError> {
  ...
  let options = Options {
    fontdb: std::sync::Arc::from(font_database),
    ..Default::default()
  };

  let tree = Tree::from_str(doc.to_string().as_str(), &options)
    .expect("Could not fetch Tree from SVG Document");
  ...
  let mut pixmap = Pixmap::new(tree.size().width() as u32, tree.size().height() as u32)
    .expect("Couldn't create a new pixmap");
  render(&tree, Transform::default(), &mut pixmap.as_mut());
  // pixmap.save_png("./arr2.png");
  match pixmap.encode_png() {
    Ok(data) => Ok(data),
    Err(err) => Err(ArrivalDisplayError::EncodingError(err)),
  }
}
